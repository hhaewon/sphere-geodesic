...
class PowerTowerFractal(DefaultManimClass):
    ...
    def construct(self):
        ...
        for i in range(20):
            ...
            sub_fractal = (
                ImageMobject(sub_tetration).scale(1 / DIV_ITER * 2).move_to((mx, my, 0))
            )
    ...
    def get_range_text(self, x, y, eps):
        ...
        text = (
            VGroup(
                MathTex("x=", f"{x:.3f} \\pm {eps_x}", color=TEAL),
        ...
        rect = (
            SurroundingRect(stroke_color=BLUE)
...
def get_subrange(tetration, criterion_div=20):
    ...
    for i in range(cx // 2):
        ...
        if criterion_div < 16:
            ...
            xy = get_subrange(tetration, criterion_div=criterion_div - 1)
...
def detect(array, x, y, pixels=N // 4, criterion_div=10):
    ...
    if (
        n_pixels // 2 - criterion
        < sub_array.astype(bool).sum()
        < n_pixels // 2 + criterion
    ):
        return x, y
    else:
...
def get_marked_image(tetration, px, py):
    ...
    sub_tetration[:, py - pys - 10 : py + pys + 10, px - pxs - 10 : px + pxs + 10] = (
        np.array([0, 255, 255], dtype=tetration.dtype)[:, None, None]
    )
    sub_tetration[:, py - pys : py + pys, px - pxs : px + pxs] = tetration[
        py - pys : py + pys, px - pxs : px + pxs
    ]
